...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(100, 250.0),  # All numbers are approximate.
                rg.Point(150, 500),
                rg.Point(100, 350),
                rg.Point(50, 300)]
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  9 green dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(300, 150), 120)
    draw_points_on_circle(window, circle, 9, 'green')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  8 slices, thin (thickness=4) purple lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 8, 'purple', 4)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        line = rg.Line(rg.Point(circle.center.x,circle.center.y), points[k])
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3:  9 segments with thick purple lines'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 100), 100)
    circle.outline_thickness = 5
    polygon(window, circle, 9, 'purple', 8)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        ...
        if k+hops_to_next_point > number_of_lines - 1:
            point2 = points[hops_to_next_point-(number_of_lines - k)]
        else:
